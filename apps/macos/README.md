...
Status: **Daily Assistant Interface built (Phase 2.2).** Not yet usable end-to-end without a Mac + Xcode to build it — see "Known limitation" below.
...
- **`Sources/AIMA/Navigation/RootNavigationView.swift`** — the navigation architecture (Phase 2.1, item 1): a `NavigationSplitView` sidebar switching between the six main screens, owning the single `WorkspaceViewModel` shared by every screen that needs to know the active workspace.
- **`Sources/AIMA/Views/`** — the six main screens:
