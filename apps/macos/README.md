...
Status: **Workflow Orchestration Foundation built (Phase 2.4).** Not yet usable end-to-end without a Mac + Xcode to build it — see "Known limitation" below.
...
- **`Sources/AIMA/Views/`** — the eight main screens:
  ...
  - `Chat/ChatView.swift` (+ `ConversationListView`, `MessageListView`, `MessageInputView`, `ApprovalCardView`, `WorkflowSuggestionCardView`) — conversation list, message history rendered as Markdown, message input, AI responses, an inline approval card (Phase 2.2, item 3) when the AI's response requires one, and an advisory workflow-suggestion card (Phase 2.4, item 4) when the message matches a built-in workflow — purely informational, it never starts a run itself.
