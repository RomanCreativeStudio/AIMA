...
/// The Tasks screen (Phase 2.2, item 4): every task in the active
/// workspace, filterable by status, each row showing its status and a
/// color-coded priority indicator — a fuller view than the Dashboard's
/// count-only "Tasks Overview" card.
...
/// A small colored dot conveying `TaskPriority` at a glance (Phase 2.2's
/// "Priority indicators") — red/orange/gray for high/medium/low, mirroring
/// how `StatusPill` conveys `ApprovalStatus`.
private struct PriorityIndicator: View {
    ...
    var body: some View {
        Circle()
            ...
            .help("Priority: \(priority.rawValue.capitalized)")
    }
}
