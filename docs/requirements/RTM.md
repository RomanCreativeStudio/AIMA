...
| `REQ-001` Authentication | `ARCH-001` §"Authentication", §9 | `ADR-0022` | `database/migrations/0020_auth_sessions.sql` | Live on every `/api` route except the public OAuth callback and the public login/refresh pair (`backend/src/app.ts`); `POST /api/auth/login`, `POST /api/auth/refresh`, `POST /api/auth/logout`, `GET /api/auth/sessions`, `DELETE /api/auth/sessions/:sessionId` now built (`backend/src/routes/auth.ts`); rate limiting still not built | `backend/src/auth/` (`types.ts`, `errors.ts`, `jwt.ts`, `mockAuthProvider.ts`, `supabaseAuthProvider.ts`, `registry.ts`, `sessionService.ts`), `backend/src/middleware/` (`auth.ts`, `workspaceOwnership.ts`, `userOwnership.ts`), `backend/src/routes/auth.ts`, `backend/src/app.ts`, `backend/src/routes/oauth.ts`, `backend/src/routes/workspaces.ts`, `backend/src/index.ts` | `backend/src/auth/jwt.test.ts`, `mockAuthProvider.test.ts`, `supabaseAuthProvider.test.ts`, `sessionService.test.ts`, `backend/src/middleware/auth.test.ts`, `workspaceOwnership.test.ts`, `backend/src/routes/auth.test.ts`, plus every `backend/src/routes/*.test.ts` (real HTTP requests with attached tokens against the live middleware stack) | [`REQ-001-authentication.md`](REQ-001-authentication.md), [`REQ-001-implementation-plan.md`](REQ-001-implementation-plan.md), [`ADR-0022`](../decisions/0022-authentication-architecture.md), [`ECIA-001`](../governance/ECIA-001-authentication-foundation.md), [`RISK-001`](../governance/RISK-001-unauthenticated-endpoints.md) |
