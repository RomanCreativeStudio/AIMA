...
**Response schema:** `ConversationService.sendMessage` returns `{ userMessage, assistantMessage, retrievedMemories, intent: { intent, confidence, approval, suggestedNextAction } }` — the AI response, the detected intent, its confidence, whether it would need approval, and what AIMA would do next if acted on.
