...
- Because Phase 2.1 was built in a Linux-only environment, `apps/Shared/AIMACore` was compiled and its 37-test suite run for real via `swift test`; the SwiftUI layer in `apps/macos` itself was written but could only be checked for manifest correctness (`swift package dump-package`) and that it fails solely on the expected `no such module 'SwiftUI'` (proving no hidden logic errors) — a genuine Xcode build/Preview pass on macOS is still outstanding.
