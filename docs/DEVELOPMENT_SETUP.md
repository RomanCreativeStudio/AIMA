...
### Semantic Search & Context Retrieval Configuration (Phase 3.6)
- No new environment variables are required — indexing and retrieval reuse the existing `EMBEDDING_PROVIDER` configuration (`mock` by default, `openai` for real embeddings) that `MemoryService`/`DocumentService` already depend on. Nothing in `backend/src/embeddings/` calls an AI provider directly, only the same injected embedding provider abstraction.
- `POST .../retrieval/reindex` never runs automatically or on a schedule — it only runs when a caller (the macOS Search screen's "Re-index Workspace" button, or a direct API call) explicitly requests it.
- `retrievedContext` on `SendMessageResult`/`GET .../retrieval/context` is `null`/empty until a workspace's conversations and tasks have actually been indexed via a reindex call — there is no backfill migration, since the `embeddings` table starts empty for every existing workspace (`database/migrations/0019_embeddings.sql` adds new tables, not new columns on existing rows).
- The two new capabilities (`semantic_search`, `reindex_embeddings`) are registered with default tiers (Tier 1/suggest and Tier 2/prepare respectively) like every other capability — no configuration flag changes their gating; see `docs/decisions/0021-semantic-search-and-context-retrieval.md`.

### Authentication Configuration (EPIC-004)
- Authentication is a provider abstraction (`AuthProvider`, `backend/src/auth/`), the same pattern as `AIProvider`/`EmbeddingProvider`/the voice providers — `AUTH_PROVIDER` defaults to `mock` (`MockAuthProvider`), so login/session flows work with zero vendor setup for local development and every existing test. Its "password" for any email is deterministic (`sha256(email).slice(0,16)`, exported as `mockPasswordFor` for tests) — convenient locally, but this must never run in production (see below).
- `"supabase"` (`SupabaseAuthProvider`, `docs/decisions/0022-authentication-architecture.md`) is the real provider — set `AUTH_PROVIDER=supabase`, `AUTH_PROVIDER_URL` (your Supabase project URL), and `AUTH_PROVIDER_API_KEY` (its anon/publishable key — safe to use here; it alone grants no database table access, see `docs/decisions/0024-database-security-boundary.md`). Missing `AUTH_PROVIDER_URL`/`AUTH_PROVIDER_API_KEY` when `AUTH_PROVIDER=supabase` fails fast at provider construction, the same fail-fast philosophy as every other required credential in this codebase.
- **`loadConfig()` refuses to start when `NODE_ENV=production` and `AUTH_PROVIDER` is unset or `mock`** (`backend/src/config/env.ts`, EPIC-005 Sprint 5.3) — unlike the other mock-defaultable providers above, a mock auth provider running in production isn't just degraded functionality, it's a silent authentication bypass, so this is the one provider selection enforced structurally rather than left to operator discipline.
- Login/refresh are rate-limited (`AUTH_LOGIN_RATE_LIMIT_MAX`/`_WINDOW_MS`, `AUTH_LOGIN_IP_RATE_LIMIT_MAX`/`_WINDOW_MS`, `AUTH_REFRESH_RATE_LIMIT_MAX`/`_WINDOW_MS`, `docs/decisions/0023-authentication-rate-limiting.md`) — all optional with production-safe built-in defaults; override only if you have a specific reason to.
