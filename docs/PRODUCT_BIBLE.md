...
| Version | Date | Change |
| --- | --- | --- |
| 2.12.0 | 2026-08-02 | EPIC-007 Sprint 7.1 (Real Google Integration Foundation): audited the entire Google/Gmail integration path (`backend/src/oauth/`, `backend/src/integrations/`) against `ADR-0022`, `ARCH-001`, and every requirement — no architectural problems found; the OAuth framework, encrypted credential storage, transparent token refresh, and capability/tier gating built in Phase 2.7 are all complete and correctly wired. Closed one real gap: connect/disconnect/rotate/OAuth-completion now write an `action_log` entry (`'prepare'` tier, no new capability), where before only the mutable `workspace_integrations` row survived a reconnect. Live-confirmed against production that `POST .../oauth/gmail/start` returns a correctly-formed Google authorization URL in every respect except `client_id`, which is confirmed to be the literal placeholder `not-yet-registered` — registered as `TD-002`, the one remaining blocker (a human OAuth-app-registration action outside this environment's scope, not a code gap) between the fully-built framework and Gmail working for a real account. Updated `ARCH-001` (v0.1.1) and `docs/PRODUCTION_SETUP.md` (v0.1.3). |
| 2.11.0 | 2026-08-02 | EPIC-006 Sprint 6.5 (Production Validation): live-verified the deployed backend end to end against `https://aima-u7c0.onrender.com` — production smoke test (12/12, twice), JWT issuer/database-connectivity cross-check confirming `AUTH_PROVIDER_URL`/`DATABASE_URL` point at the correct Supabase project, CORS lockdown, security headers, `/health` stability, and Supabase `postgres`/`auth` logs (no unexplained errors). Found and recorded `TD-001` (first Technical Debt Register entry): production is deployed from `claude/aima-product-bible-48an4f`, not `main`, which contains only the repository's original initial commit — no branch protection, no CI. Updated `docs/PRODUCTION_SETUP.md` (v0.1.2) to reflect what Sprint 6.1/6.4/6.5 actually verified, rather than leaving its checklist boxes unchecked after deployment had already happened. One item explicitly not live-verified this sprint: a brand-new Supabase Auth account's first-login auto-provisioning round-trip, blocked by Supabase's own signup email-rate-limit — code-level coverage (9 tests, Sprint 6.4) stands in its place. |
...
| Requirements Index | `REQ-INDEX` | [`docs/requirements/REQ-INDEX.md`](requirements/REQ-INDEX.md) | Active. Canonical index of every requirement's status and file; 5 entries. |
...
| Authentication Implementation Planning | `REQ-001-PLAN` | [`docs/requirements/REQ-001-implementation-plan.md`](requirements/REQ-001-implementation-plan.md) | Active. Planning artifact expanding `REQ-001`, written before any provider/schema decision; superseded in practice by `ADR-0022`/`ADR-0023`, which made and recorded those decisions, and by `REQ-001` (now `Implemented`). Preserved as the pre-implementation planning record, not rewritten. |
| ECIA Index | `ECIA-INDEX` | [`docs/governance/ECIA-INDEX.md`](governance/ECIA-INDEX.md) | Active. Canonical index of every Engineering Change Impact Analysis record; 1 entry (`ECIA-001`). |
...
| Risk Register | `RISK-*` | [`docs/governance/`](governance/) | Framework active (`RISK-INDEX`/`RISK-TEMPLATE`); 1 risk registered (`RISK-001`). |
| Risk Register Index | `RISK-INDEX` | [`docs/governance/RISK-INDEX.md`](governance/RISK-INDEX.md) | Active. Canonical index of every risk entry's status, severity, and file; 1 entry (`RISK-001`). |
...
| Technical Debt Register | `TD-*` | [`docs/governance/`](governance/) | Framework active (`TD-INDEX`/`TD-TEMPLATE`); 2 debt items registered (`TD-001`, `TD-002`). |
| Technical Debt Register Index | `TD-INDEX` | [`docs/governance/TD-INDEX.md`](governance/TD-INDEX.md) | Active. Canonical index of every technical debt entry's status, priority, and file; 2 entries (`TD-001`, `TD-002`). |
...
The canonical Technical Debt Register now lives in [`docs/governance/TD-INDEX.md`](governance/TD-INDEX.md) (canonical index) and [`docs/governance/TD-TEMPLATE.md`](governance/TD-TEMPLATE.md) (`TD-TEMPLATE`, the authoring template). This handbook's original one-line placeholder was extended, not deleted, into the full framework — per this handbook's own Documentation Review Workflow: preserve existing useful content, move it rather than delete it. The first real entry, `TD-001` (production deploys from a long-lived feature branch, not `main` — no branch protection, no CI), was recorded during EPIC-006 Sprint 6.5's live production validation.
