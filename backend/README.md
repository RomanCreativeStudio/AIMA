...
- **`src/routes/`** — HTTP endpoints. `health.ts` reports the `HealthService` result; `capabilities.ts` is foundation-stage; `ai.ts` is an explicitly-labeled smoke-test endpoint predating the real pipeline; `memories.ts` wires memory creation through the `PermissionEngine`/`ActionLogger` and exposes search + workspace-context retrieval; `documents.ts` exposes the document lifecycle API; `tasks.ts` exposes the Task Foundation CRUD API; `conversations.ts` exposes conversation creation, history, and message sending (the real pipeline, now delegating to `AimaCoreService`). `ApprovalEngine` has no route yet — see `docs/decisions/0004-intent-and-approval-engine.md` for why.
