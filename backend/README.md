...
- **`src/core/`** — the Assistant Core Orchestration Layer (Phase 1.6, docs/decisions/0006-assistant-core-orchestration.md). `aimaCoreService.ts` (`AimaCoreService`) is the coordinator: gather context, detect intent, resolve the intent's mapped capability and call `ApprovalEngine.evaluate` with its extracted parameters (Phase 1.7), build the system prompt, call the AI provider, return a structured response — takes conversation history as a plain array, with no knowledge of `conversations`/`messages` tables. `contextManager.ts` (`ContextManager`) combines memory, document, and (pass-through) history context with configurable `memoryLimit`/`documentLimit`. `assistantProfiles.ts` defines per-workspace `AssistantProfile`s (Personal, Roman Creative Studio, Mythic Forge Studios, Development), each with real `responseInstructions`, not just a description. `contextAssembly.ts` (moved here from `conversation/`) builds the system prompt from an `AssistantProfile` plus memory/document context. `types.ts` defines `UnifiedContext`/`ContextLimits`.
...
- Workspace CRUD and per-workspace permission tier overrides (`workspace_capability_settings`) — capability tiers currently resolve to their code-registered default only, so `AimaCoreService`'s automatic approval creation (Phase 1.7) has no real Tier 3 capability to exercise yet under shipped defaults.
