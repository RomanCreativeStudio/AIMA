...
| `0022_feedback_status.sql` | Adds a `feedback_status` enum (`new`/`reviewed`/`resolved`) and a `status` column (default `new`) to `feedback` — the Internal Operator Dashboard sprint's one schema change, needed because `feedback` had no lifecycle/status concept at all before this. Display-only this sprint (no mutation endpoint yet). Purely additive; rollback is `ALTER TABLE feedback DROP COLUMN status; DROP TYPE IF EXISTS feedback_status;`. |
| `0023_invitations.sql` | Adds an `invitation_status` enum (`pending`/`accepted`/`expired`) and the `invitations` table (`email`, `invited_by` FK to `users`, `status`, `created_at`, a monotonic `sequence` column from the start) — the Beta Invitations & Notifications sprint's founder-issued invite tracking. Unlike every prior admin sprint, this is genuinely new data rather than something reusable from `users.preferences` (the invitee usually has no `users` row yet). Purely additive; rollback is `DROP TABLE IF EXISTS invitations; DROP TYPE IF EXISTS invitation_status;`. |
| `0024_task_source_metadata.sql` | Adds nullable `source` and `metadata` (`JSONB`, default `'{}'`) columns to `tasks` — the Conversation → Action sprint's way of recording that a task was accepted from a chat suggestion rather than typed in by hand, mirroring the exact `source`/`metadata` shape `memory_records` already uses (`0017_memory_intelligence.sql`). Purely additive; rollback is `ALTER TABLE tasks DROP COLUMN source, DROP COLUMN metadata;`. |
